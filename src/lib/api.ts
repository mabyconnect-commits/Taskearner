...
export const api = {
  ...
  addBank: (b: { bankName: string; bankCode?: string; accountNumber: string; accountName: string }) => req("/bank", "POST", b),
  ...
  leaderboard: () => req<{ leaderboard: { name: string; handle: string; earned: number; refs: number; activeRefs?: number }[] }>("/leaderboard"),
